    ...
    <div className="min-h-screen bg-slate-50 font-lexend overflow-x-hidden pb-20 lg:pb-0">
      <nav className="bg-white border-b border-slate-200 px-4 md:px-8 py-3 md:py-4 flex justify-between items-center sticky top-0 z-30 shadow-sm">
        <div className="flex items-center gap-2 md:gap-3">
          <div className="w-8 h-8 md:w-10 md:h-10 bg-primary text-white rounded-xl flex items-center justify-center shadow-lg shadow-indigo-100">
            <BookOpen size={18} />
          </div>
          <span className="text-xl md:text-2xl font-black text-primary tracking-tighter">SchoolSync</span>
        </div>
        <div className="flex items-center gap-3 md:gap-8">
          <button
            onClick={toggleLanguage}
            className="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 px-4 py-2 rounded-full transition-all text-xs font-bold text-slate-700 border border-slate-200"
          >
          ...
          <div
            ...
            className="w-10 h-10 md:w-12 md:h-12 bg-slate-100 rounded-full flex items-center justify-center text-primary border-2 border-white shadow-sm cursor-pointer hover:ring-2 hover:ring-primary transition-all overflow-hidden"
          >
            <User size={20} />
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {exams.map(exam => (
                    <div
                      ...
                      className={`p-6 rounded-[2rem] border-2 cursor-pointer transition-all relative group ${selectedExam?.id === exam.id
                    ...
                    >
                      <div className="absolute top-4 right-4 flex gap-2">
                        <button
                          ...
                          className={`p-2 rounded-xl opacity-0 group-hover:opacity-100 transition-all ${selectedExam?.id === exam.id ? 'bg-white/20 text-white hover:bg-white/30' : 'bg-primary/5 text-primary hover:bg-primary/10'
                            }`}
                        >
                          <Edit size={16} />
                        </button>
                        <button
                          ...
                          className={`p-2 rounded-xl opacity-0 group-hover:opacity-100 transition-all ${selectedExam?.id === exam.id ? 'bg-white/20 text-white hover:bg-white/30' : 'bg-red-50 text-red-400 hover:bg-red-100'
                            }`}
                        >
                          <Trash size={16} />
                        </button>
                  ...
                  <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-2xl overflow-hidden animate-in zoom-in duration-300">
                    ...
                    <div className="overflow-x-auto pb-4">
                      ...
                      <table className="w-full text-left min-w-[800px]">
                        <thead className="bg-slate-50 border-b border-slate-100">
                          <tr>
                            <th className="px-8 py-5 text-xs font-black text-slate-400 uppercase tracking-widest">{t('student')}</th>
                            <th className="px-8 py-5 text-xs font-black text-slate-400 uppercase tracking-widest">{t('add_marks')}</th>
                            <th className="px-8 py-5 text-xs font-black text-slate-400 uppercase tracking-widest">{t('current_scores')}</th>
                            <th className="px-8 py-5 text-xs font-black text-slate-400 uppercase tracking-widest">{t('total_percentage')}</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                          {students.map(student => {
                            const studentResults = currentResults.filter(r => r.student === student.id);
                            return (
                              <tr key={student.id} className="hover:bg-slate-50/50 transition-colors">
                                <td className="px-8 py-6">
                                  <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center text-primary font-black text-sm overflow-hidden border border-white shadow-sm">
                                      {student.profile_picture ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {exams.map(exam => (
                    <div
                      key={exam.id}
                      onClick={() => setSelectedExam(exam)}
                      className={`group p-6 rounded-[2rem] border-2 transition-all cursor-pointer relative ${selectedExam?.id === exam.id
